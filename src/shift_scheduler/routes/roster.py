...
from shift_scheduler.auth import client_ip, require_editor
...
def roster_create(
    ...
    write_log(
        db,
        "person_create",
        {"person_id": person.id, "name": person.name, "role": person.role},
...
def period_create(
    ...
    note_value = (note.strip() if note else None) or None
    pp = PresencePeriod(
        person_id=person_id,
        start_date=sd,
        end_date=ed,
...
def period_delete(
    ...
    write_log(
        db,
        "period_delete",
        {"person_id": person_id, "period_id": period_id},
        actor_ip=client_ip(request),
    )
    db.delete(pp)
    db.commit()
